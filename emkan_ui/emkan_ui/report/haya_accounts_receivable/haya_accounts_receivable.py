	...
	def get_invoice_details(self):
		...
		if self.account_type == "Receivable":
			si_list = frappe.db.sql(
				"""
				SELECT name, due_date, po_no, remarks, customer_name
			...
			for d in si_list:
				d.doc_ref = f"PO: {d.po_no}" if d.po_no else (f"Remarks: {d.remarks}" if d.remarks else "")
				d.voucher_type = "Credit Note" if d.is_return else "Sales Invoice"
		...
		if self.account_type == "Payable":
			pi_list = frappe.db.sql(
				"""
				SELECT name, due_date, bill_no, bill_date, supplier_name
			...
			for pi in pi_list:
				pi.customer_name = pi.supplier_name
				pi.doc_ref = f"Bill: {pi.bill_no}" if pi.bill_no else ""
				self.invoice_details.setdefault(pi.name, pi)

		# Journal Entries
		journal_entries = frappe.db.sql(
			"""
			SELECT name, due_date, bill_no, bill_date, user_remark
		...
		for je in journal_entries:
			if je.bill_no:
				je.customer_name = ""
				je.doc_ref = je.user_remark or f"Bill: {je.bill_no}"
				self.invoice_details.setdefault(je.name, je)

		# Payment Entries
		payment_entries = frappe.db.sql(
			"""
			SELECT name, posting_date as due_date, reference_no, remarks, party_name
		...
		for pe in payment_entries:
			pe.customer_name = pe.party_name
			pe.doc_ref = pe.reference_no or (f"Remarks: {pe.remarks}" if pe.remarks else "")
			self.invoice_details.setdefault(pe.name, pe)
