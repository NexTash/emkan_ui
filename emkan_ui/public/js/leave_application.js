	...
	add() {
        ...
        if (!me.assign_to) {
            me.assign_to = new frappe.ui.form.AssignToDialog({
                method: "emkan_ui.events.leave_application.add", // ✅ our custom method
                ...
                callback: function (r) {
                    me.render(r.message);
                    frappe.show_alert({
                        message: __("Assignment created with Role saved."),
                        indicator: "green",
                    });
                },
            });
				...
				if (args && args.assign_to) {
					...
					frappe.call({
						...
						args: $.extend(args, {
							...
							role: args.role, // send role to backend
	...
	make() {
		this.dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "HTML", fieldname: "assignment_list" },
			],
		});
	...
	get_assignment_row(assignment) {
		const row = $(`
			<div class="dialog-assignment-row" data-user="${assignment}">
				...
				<div class="btn-group btn-group-sm" role="group" aria-label="Actions"></div>
			</div>
		`);
